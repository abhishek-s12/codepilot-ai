...
def get_repository_analytics(repo_path: str):
    ...
    avg_complexity = (
        round(sum(complexities) / len(complexities), 1) if complexities else 1
    )

    # Largest Files & Folders sorting
    largest_files = sorted(
        largest_files_candidates, key=lambda x: x["size"], reverse=True
    )[:5]
    largest_folders = sorted(
        [
            {"path": k, "size": v, "count": folder_file_counts[k]}
    ...
    result = {
        ...
        "complexity_histogram": [
            {"range": k, "count": v} for k, v in complexity_histogram.items()
        ],
