...
def log_event(
    ...
    """Logs a backend telemetry event to PostgreSQL database."""
    try:
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute(
...
def get_telemetry_metrics():
    ...
    try:
        ...
        # 3. Total AI Requests
        cursor.execute(
            "SELECT COUNT(*) FROM telemetry_logs WHERE event_type LIKE 'ai_%%'"
        )
        ...
        # 5. Error Rate
        cursor.execute(
            "SELECT COUNT(*), SUM(CASE WHEN success = 0 THEN 1 ELSE 0 END) FROM telemetry_logs"
        )
        row = cursor.fetchone()
        if row:
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM telemetry_logs WHERE event_type = 'cache_hit'"
        )
        hits = cursor.fetchone()[0]
        cursor.execute(
            "SELECT COUNT(*) FROM telemetry_logs WHERE event_type = 'cache_miss'"
        )
        ...
        cursor.execute(
            "SELECT event_type, latency, success, error_message, timestamp FROM telemetry_logs ORDER BY timestamp DESC LIMIT 5"
        )
