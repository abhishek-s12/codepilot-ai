...
def mock_db_and_observability():
    ...
    with patch("tasks.indexing_tasks.update_repository_status") as mock_status, \
...
def test_index_repository_task_happy_path(mock_db_and_observability):
    ...
    mock_steps = [
        ...
        {"stage": "Completed", "progress": 100, "message": "Completed", "data": {"files_indexed": 12, "chunks_indexed": 45}},
    ]

    with patch("tasks.indexing_tasks.index_repository_generator", return_value=mock_steps) as mock_gen, \
        ...
        assert mock_progress.call_count == len(mock_steps) + 1  # 1 initial + len(mock_steps)
...
def test_index_repository_task_failure(mock_db_and_observability):
    ...
    with patch("tasks.indexing_tasks.index_repository_generator", side_effect=ValueError("Disk Full")) as mock_gen:
...
def test_archive_and_upload_task():
    ...
    with patch("services.repo_service.archive_and_upload_repo", return_value=True) as mock_service:
