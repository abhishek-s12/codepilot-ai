...
def test_knowledge_graph_builder():
    ...
    print(
        f"SUCCESS: Knowledge Graph builder saved {len(res['nodes'])} nodes and {len(res['edges'])} edges."
    )
...
def test_task_planner():
    ...
    print(
        f"SUCCESS: Planner generated plan with {len(plan['tasks'])} sequential tasks."
    )
...
def test_devops_generator():
    ...
    assert (
        ...
        or "RUN" in dockerfile_content
        or "COPY" in dockerfile_content
    )
