...
type PaletteItem = Command | FileItem;
...
interface CommandPaletteProps {
  ...
  onExecuteCommand: (key: string) => void;
}
...
function isCommand(item: PaletteItem): item is Command {
  return "key" in item;
}
...
export default function CommandPalette({
      ...
      if (repoPath) {
        setTimeout(() => {
          ...
          fetchRepositoryFiles(repoPath)
            .then((res) => {
              const apiResult = res as { files?: FileItem[] };
              setFiles(apiResult?.files || []);
            })
            .catch((err) => {
  ...
  const filteredItems = useMemo((): PaletteItem[] => {
    if (isCommandMode) {
      ...
      if (!cleanQuery) return files.slice(0, 15);
      return files
        .filter((file) => {
  ...
  const handleSelectItem = (item: PaletteItem) => {
    if (isCommandMode && isCommand(item)) {
      onExecuteCommand(item.key);
    } else if (!isCommandMode && !isCommand(item)) {
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-3.5 border-b border-white/5 shrink-0 bg-black/20">
          <span className="text-gray-500 font-sans text-sm shrink-0" aria-hidden="true">
            {isCommandMode ? ">" : "🔍"}
          </span>
          <input
            ref={inputRef}
        ...
        <div className="max-h-[340px] overflow-y-auto min-h-0 flex-grow scrollbar-thin">
          {loading ? (
            <div className="p-8 text-center text-gray-500 flex items-center justify-center gap-2">
              <span className="w-4 h-4 border-2 border-accent/20 border-t-accent rounded-full animate-spin"></span>
            ...
            <div ref={listRef} className="p-1.5 space-y-0.5" role="listbox">
              {filteredItems.map((item, idx) => {
                const isSelected = activeIndex === idx;
                const itemKey = isCommand(item) ? item.key : item.path;
                const itemLabel = isCommand(item) ? item.label : item.path.replace(/\\/g, "/").split("/").pop() ?? item.path;
                const itemIcon = isCommand(item) ? item.icon : (item.icon ?? "📄");
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between text-left px-3 py-2.5 rounded-xl font-sans text-xs transition-all ${
                      isSelected
                        ? "bg-accent text-bg text-white font-semibold"
