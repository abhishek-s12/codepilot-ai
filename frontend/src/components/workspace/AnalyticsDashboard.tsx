...
export default function AnalyticsDashboard({ onBack }: AnalyticsDashboardProps) {
  ...
  const loadAllAnalytics = async () => {
    try {
      setLoading(true);
      setError(null);
      const [tData, cData, aData] = await Promise.all([
        fetchTokenUsage(),
        fetchCodebaseAnalytics(),
        fetchWorkspaceActivity()
      ]);
  ...
  const getX = (index: number) => paddingLeft + (index / (tokens.length - 1)) * chartWidth;
    ...
    <div className="flex-1 overflow-y-auto p-8 space-y-8 bg-[#0A0D12]">
      ...
      <div className="flex justify-between items-center border-b border-[#1E2530] pb-6">
        ...
        <div className="flex items-center space-x-3">
          <button 
            onClick={loadAllAnalytics}
            className="flex items-center space-x-2 px-4 py-2 text-xs font-semibold rounded-lg bg-[#10141B] border border-[#1E2530] text-gray-300 hover:text-white hover:bg-[#161C26] hover:border-[#FF9D4D]/50 transition-all cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-[#10141B]/60 border border-[#1E2530] p-6 rounded-xl relative overflow-hidden group hover:border-[#FF9D4D]/40 transition-all">
          ...
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest">Developer Hours Saved</p>
          <p className="text-3xl font-extrabold text-white mt-2">
            ~{Math.round((codebase.total_chunks * 0.15) + 12)} hrs
          </p>
          <p className="text-xs text-gray-500 mt-2">Estimated autonomous savings</p>
        </div>
      </div>
          ...
          <div className="bg-[#10141B]/40 border border-[#1E2530] p-6 rounded-xl backdrop-blur-md">
            ...
            <div className="relative">
              <svg className="w-full overflow-visible" height={svgHeight} viewBox={`0 0 ${svgWidth} ${svgHeight}`}>
                ...
                {/* Interactive Circles / Hover Bars */}
                {tokens.map((t, idx) => (
                  <g key={idx}>
                    <line 
                      x1={getX(idx)}
                      ...
                      stroke={hoveredToken?.date === t.date ? "#FF9D4D/40" : "transparent"}
                      strokeWidth={1.5}
                    />
                    <circle 
                      cx={getX(idx)}
                      cy={getY(t.input_tokens)}
                      r={hoveredToken?.date === t.date ? 6 : 4}
                      fill="#0A0D12"
                      stroke="#FF9D4D"
                      strokeWidth={2}
                    ...
                    <circle 
                      ...
                      r={hoveredToken?.date === t.date ? 5 : 3.5}
                      fill="#0A0D12"
                      stroke="#52D3A2"
                      strokeWidth={1.5}
                      className="cursor-pointer transition-all"
                ...
                {/* X Axis Labels (Dates) */}
                {tokens.map((t, idx) => {
                  // Show every 3rd date label to prevent crowding
              ...
              {hoveredToken && (
                <div 
                  className="absolute bg-[#10141B] border border-[#1E2530] p-3 rounded-lg shadow-2xl backdrop-blur-md pointer-events-none transition-all duration-75"
                  style={{
                    left: `${Math.min(getX(tokens.indexOf(hoveredToken)) - 50, 460)}px`,
                    top: "10px"
                  }}
                >
                  <p className="text-[10px] font-bold text-[#FF9D4D] tracking-wide mb-1 uppercase">
                    {new Date(hoveredToken.date).toLocaleDateString(undefined, { weekday: "short", month: "short", day: "numeric" })}
                  </p>
                  <div className="space-y-1 text-xs">
                    <p className="text-gray-300 flex justify-between space-x-6">
                      <span>Input Context:</span>
                      <span className="font-mono text-white font-bold">{hoveredToken.input_tokens.toLocaleString()}</span>
                    </p>
                    <p className="text-gray-300 flex justify-between space-x-6">
                      <span>Model Response:</span>
                      <span className="font-mono text-white font-bold">{hoveredToken.output_tokens.toLocaleString()}</span>
                    </p>
                    <p className="text-[#52D3A2] flex justify-between space-x-6 pt-1 border-t border-[#1E2530]">
                      <span>Requests Made:</span>
                      <span className="font-mono font-bold">{hoveredToken.requests}</span>
                    </p>
                  </div>
                </div>
              )}
            ...
            <div className="relative">
              <div className="flex flex-wrap gap-1.5 max-h-[140px] overflow-visible">
                {activity.map((act, index) => (
                  <div 
                    key={index}
                    className={`w-[13px] h-[13px] rounded-sm border cursor-pointer transition-all duration-150 ${getHeatmapColor(act.count)}`}
              ...
              {hoveredActivity && (
                <div 
                  className="absolute bg-[#10141B] border border-[#1E2530] px-3 py-1.5 rounded shadow-xl text-[10px] text-white backdrop-blur-md pointer-events-none transform -translate-x-1/2 -translate-y-full"
                  style={{
                    left: `${Math.min(Math.max(30, (activity.indexOf(hoveredActivity) % 45) * 19.5), 520)}px`,
                    top: `${Math.floor(activity.indexOf(hoveredActivity) / 45) * 19.5 - 5}px`
                  }}
                >
                  <span className="font-bold text-[#FF9D4D]">{hoveredActivity.count} events</span> on {new Date(hoveredActivity.date).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" })}
                </div>
              )}
            </div>
          </div>
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="relative w-44 h-44 mb-6">
                <svg className="w-full h-full transform -rotate-90" viewBox="0 0 42 42">
                  ...
                  {/* Accumulate stroke offsets for segments */}
                  {(() => {
                    let offsetAccumulator = 0;
                    return codebase.file_ratios.map((ratio, index) => {
                      const strokeDash = `${ratio.value} ${100 - ratio.value}`;
                      const currentOffset = offsetAccumulator;
                      offsetAccumulator += ratio.value;
              ...
              <div className="w-full space-y-2">
                {codebase.file_ratios.map((ratio, idx) => (
                  <div 
                    key={idx}
                    className={`flex justify-between items-center px-2 py-1.5 rounded-lg border transition-all ${hoveredRatio === idx ? "bg-[#161C26] border-[#FF9D4D]/35" : "border-transparent"}`}
                  ...
                  >
                    ...
                    <span className="text-xs font-bold text-white font-mono">{ratio.value}%</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-[#10141B]/40 border border-[#1E2530] p-6 rounded-xl backdrop-blur-md">
            ...
            <div className="space-y-4">
              ...
              <div className="p-3 bg-[#10141B]/60 border border-[#1E2530] rounded-xl flex justify-between items-center hover:border-[#FF9D4D]/25 transition-all">
                ...
                <span className="font-mono text-sm font-bold text-gray-300">{codebase.symbols.classes}</span>
              </div>
              ...
              <div className="p-3 bg-[#10141B]/60 border border-[#1E2530] rounded-xl flex justify-between items-center hover:border-[#FF9D4D]/25 transition-all">
                ...
                <span className="font-mono text-sm font-bold text-gray-300">{codebase.symbols.functions}</span>
              </div>
              ...
              <div className="p-3 bg-[#10141B]/60 border border-[#1E2530] rounded-xl flex justify-between items-center hover:border-[#FF9D4D]/25 transition-all">
                ...
                <span className="font-mono text-sm font-bold text-gray-300">{codebase.symbols.modules}</span>
              </div>
            </div>
          </div>
