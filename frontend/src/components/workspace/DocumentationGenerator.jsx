...
export default function DocumentationGenerator({ activeFile, onTriggerAction }) {
    ...
    <div className="space-y-4 select-none text-left font-sans">
      <div className="border-b border-[#1c2230]/40 pb-2">
        <h3 className="text-xs font-bold text-white font-mono uppercase tracking-wider flex items-center gap-1.5">
          <BookOpen className="w-4 h-4 text-indigo-400" /> AI Documentation Generator
        </h3>
        <p className="text-[10px] text-gray-500 font-sans">Build README files, inline comments, or API references docs.</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-2.5">
        {[
          ...
          <button
            ...
            className="p-3 bg-[#141822] border border-[#1c2230] hover:border-indigo-500/25 rounded-xl text-left flex justify-between gap-3 items-center transition-all cursor-pointer group"
          >
            <div className="space-y-0.5 truncate text-[10px] font-mono">
              <span className="font-bold text-white block group-hover:text-indigo-400 transition-colors">{item.label}</span>
              <span className="text-gray-500 text-[9px] block truncate">{item.desc}</span>
            </div>
            <ArrowRight className="w-4 h-4 text-gray-600 group-hover:translate-x-0.5 transition-all shrink-0" />
          </button>
        ))}
      </div>
    </div>
