...
export default function MemoryManager({ onOpenFile, onTriggerAction }: MemoryManagerProps) {
  return (
    <div className="space-y-4 select-none text-left font-sans">
      <div className="border-b border-[#1c2230]/40 pb-2">
        <h3 className="text-xs font-bold text-white font-mono uppercase tracking-wider flex items-center gap-1.5">
          <RefreshCw className="w-3.5 h-3.5 text-accent" /> Session Resume Memory
        </h3>
      ...
      <div className="space-y-2.5 max-h-[220px] overflow-y-auto scrollbar-thin pr-1">
        {RECENT_SESSIONS.map((session, idx) => (
          <div key={idx} className="p-3 bg-[#141822] border border-[#1c2230] hover:border-accent/10 rounded-xl flex items-center justify-between gap-3 text-[10px] font-mono transition-all">
            ...
            <div className="flex gap-1.5 shrink-0">
              {onOpenFile && (
                <button
                  ...
                  aria-label={`Open file: ${session.file}`}
              ...
              {onTriggerAction && (
                <button
                  ...
                  className="p-1.5 rounded-lg bg-accent text-bg hover:bg-accent-strong text-white font-bold cursor-pointer"
                >
