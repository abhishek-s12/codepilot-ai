...
const DEFAULT_PINNED: PinnedChat[] = [
...
function loadPinned(): PinnedChat[] {
  const saved = localStorage.getItem("workspace-pinned-chats");
  if (saved) {
    try {
      return JSON.parse(saved) as PinnedChat[];
    } catch {
...
export default function PinnedChats() {
  const [pinned, setPinned] = useState<PinnedChat[]>(loadPinned);

  const savePinned = (updated: PinnedChat[]) => {
    setPinned(updated);
    ...
    <div className="space-y-4 select-none text-left font-sans">
      <div className="border-b border-[#1c2230]/40 pb-2">
        <h3 className="text-xs font-bold text-white font-mono uppercase tracking-wider flex items-center gap-1.5">
          <Pin className="w-4 h-4 text-accent" /> Pinned AI Chats
        </h3>
      ...
      <div className="space-y-2 max-h-[260px] overflow-y-auto scrollbar-thin pr-1">
        {pinned.length === 0 ? (
          ...
          pinned.map((item) => (
            <div
              key={item.id}
              className="p-3 bg-[#141822] border border-[#1c2230] hover:border-accent/20 rounded-xl relative group flex justify-between items-start gap-3 transition-all"
            >
