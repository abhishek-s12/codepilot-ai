    ...
    <div className="space-y-6 animate-fade-in w-full h-[580px] flex flex-col">
      ...
      <div className="border-b border-border pb-4 shrink-0 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold tracking-tight text-text-strong flex items-center gap-2 font-display">
            <IconCpu className="w-5 h-5 text-accent" /> Interactive Dependency Graph
          </h2>
        ...
        <div className="flex items-center gap-3">
          ...
          {architecture && (
            <div className="relative">
              ...
              <input
                ...
                className="pl-8 pr-3 py-1.5 rounded-lg text-xs bg-bg border border-border text-text-strong placeholder-muted focus:outline-none focus:border-accent w-[160px] transition-all font-mono shadow-inner"
              />
              {searchQuery && (
                <button
          ...
          {!architecture && (
            <button
              ...
              className="px-5 py-2.5 text-xs font-semibold bg-accent text-bg hover:bg-accent-strong rounded-xl transition-all inline-flex items-center gap-1.5 cursor-pointer font-mono shadow-md"
            >
              {isArchitectureLoading ? "Tracing Architecture..." : "Load Graph"}
            </button>
          )}
        </div>
      </div>
      ...
      {architecture ? (
        <div className="flex-grow grid grid-cols-1 xl:grid-cols-[1fr_300px] gap-4 min-h-0">
          ...
          {/* File Details Drawer */}
          <div className="p-4 rounded-2xl border border-border bg-panel flex flex-col justify-between h-full xl:h-[430px] overflow-y-auto shadow-lg">
            {showReport ? (
              <div className="space-y-4 flex-grow">
                <div className="pb-3 border-b border-border">
                  <h4 className="text-sm font-bold text-text-strong font-mono">
                    Senior Architect Report
                  </h4>
                </div>
                <div className="text-xs text-text font-sans max-h-[290px] overflow-y-auto pr-1 space-y-2 scrollbar-thin">
              ...
              <div className="space-y-4">
                <div className="pb-3 border-b border-border">
                  ...
                  <h4 className="text-sm font-bold text-text-strong font-mono truncate mt-2">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 py-2 bg-accent hover:bg-accent-strong rounded-lg text-[10px] text-bg font-bold transition-all cursor-pointer font-mono"
                  >
                    Explain File
                  </button>
                  <button
                    onClick={() => onOpenFile(selectedNode.id)}
                    className="flex-1 py-2 bg-panel-alt hover:bg-panel-alt-2 border border-border rounded-lg text-[10px] text-text font-medium transition-all cursor-pointer font-mono"
                  >
                    View Content
                  </button>
                </div>

                <div className="p-3 bg-panel-alt rounded-xl border border-border space-y-1.5 text-[11px] text-text">
                  <p className="font-semibold text-secondary font-display">Import Dependencies:</p>
                  <div className="space-y-1 font-mono max-h-[140px] overflow-y-auto pr-1 scrollbar-thin">
                    {graphEdges.filter(e => e.source === selectedNode.id).length === 0 ? (
                      <p className="italic text-muted">No outgoing imports.</p>
                    ) : (
                      graphEdges.filter(e => e.source === selectedNode.id).map(e => (
                        <p key={e.id} className="truncate text-text">→ {e.target?.split("/").pop()}</p>
                      ))
                    )}
                  </div>
                </div>
              ...
              <div className="flex items-center justify-center h-full text-center p-4">
                <p className="text-xs text-muted italic font-body">
                  Select a node in the graph to inspect details, or read the architect report.
                </p>
              </div>
            )}
            ...
            <div className="mt-4 pt-3 border-t border-border shrink-0">
              <button
                ...
                className="w-full py-2 bg-panel hover:bg-panel-alt rounded-lg text-[10px] font-semibold text-soft border border-border hover:border-accent/40 transition-all flex items-center justify-center gap-1 cursor-pointer font-mono"
              >
        ...
        <div className="py-24 text-center text-muted space-y-4 w-full">
          ...
          <p className="text-sm font-body">Architecture graph not compiled yet.</p>
          <button
            onClick={onGetArchitecture}
            disabled={isArchitectureLoading}
